...
$bookings = $con->query("SELECT * FROM bookings ORDER BY rental_start_date ASC");
$userlogs = $con->query("SELECT * FROM user_logs");
...
// Handle the complete action (move booking to history)
if (isset($_GET['complete_id'])) {
    ...
    if ($booking) {
        // Move data from bookings to history
        $stmt = $con->prepare("INSERT INTO history (user_id, item_id, item_name, price_per_day, days_rented, total_cost, payment_status, payment_method, booking_date, rental_start_date, delivery_address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param(
            "sissdssssss",
            $booking['user_id'],
            $booking['item_id'],
        ...
        // Delete the booking from the bookings table after moving to history
        $stmt = $con->prepare("DELETE FROM bookings WHERE booking_id = ?");
        $stmt->bind_param("i", $booking_id);
        $stmt->execute();
        $stmt->close();

        $alert_message = "Booking has been marked as complete and moved to history.";
        $alert_type = "success";
    } else {
        $alert_message = "Booking not found.";
        $alert_type = "danger";
    }
    echo "<script>setTimeout(function(){ window.location.href = 'admin_dashboard.php'; }, 2000);</script>";
...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="#" id="userlogsLink">User Logs</a>
    ...
    <div class="content">
        ...
        <div id="rentalsSection" class="section" style="display: none;">
            <a href="#" id="addRental" class="btn btn-success float-right">Add Rental</a>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($rental['item_image']): ?>
                                    <img src="data:image/jpeg;base64,<?php echo base64_encode($rental['item_image']); ?>"
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="admin_dashboard.php?action=delete_booking&id=<?php echo $booking['booking_id']; ?>"
                                    class="btn btn-danger btn-sm">Delete</a>
                                <a href="admin_dashboard.php?complete_id=<?php echo $booking['booking_id']; ?>"
        ...
        <div id="historySection" class="section" style="display: none;">
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <?php
                    // Fetch history
                    $history = $con->query("SELECT * FROM history ORDER BY completed_date DESC");
                    while ($history_item = $history->fetch_assoc()) {
                        echo "<tr>";
                        echo "<td>{$history_item['history_id']}</td>";
                        echo "<td>{$history_item['user_id']}</td>";
                        echo "<td>{$history_item['item_name']}</td>";
                        echo "<td>{$history_item['price_per_day']}</td>";
                        echo "<td>{$history_item['total_cost']}</td>";
                        echo "<td>{$history_item['payment_status']}</td>";
                        echo "<td>{$history_item['rental_start_date']}</td>";
                        echo "<td>{$history_item['completed_date']}</td>";
                        echo "</tr>";
                    }
                    ?>
                </tbody>
        ...
        <div id="userlogsSection" class="section" style="display: none;">
            <h2>User Logs</h2>
            <table class="table table-bordered">
                <thead class="thead-light">
                    <tr>
                ...
                <tbody>
                    ...
                    <?php
                    while ($userlog = $userlogs->fetch_assoc()): ?>
                        <tr>
    ...
    <script>
        $(document).ready(function () {
            ...
            var rentalStatsChart = new Chart(ctx, {
                type: 'bar',
                data: {
                    labels: ['Total Rentals', 'Total Bookings'],
                    datasets: [{
                        label: 'Statistics',
                        data: [<?php echo $totalRentals; ?>, <?php echo $totalBookings; ?>],
                        backgroundColor: ['#66bb6a', '#42a5f5'],
                        borderColor: ['#388e3c', '#1e88e5'],
                        borderWidth: 1
                    }]
                },
                options: {
    ...
    <!-- Edit Rental Modal -->
    <div class="modal fade" id="editRentalModal" tabindex="-1" aria-labelledby="editRentalModalLabel"
        aria-hidden="true">
                ...
                <form action="admin_dashboard.php" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="editPricePerDay" name="price_per_day"
                                required>
                        </div>
                        <div class="form-group">
